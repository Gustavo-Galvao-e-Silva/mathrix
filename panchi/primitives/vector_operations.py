...
def dot(vector_1: Vector, vector_2: Vector) -> float:
    ...
    ------
    TypeError
        If either argument is not a Vector, or if the vectors have different dimensions.
    ValueError
    ...
    32
    """
    if not isinstance(vector_1, Vector) or not isinstance(vector_2, Vector):
        raise TypeError(
            f"Both arguments must be Vector instances. "
            f"Got vector_1: {type(vector_1).__name__}, vector_2: {type(vector_2).__name__}."
        )

    if vector_1.dims != vector_2.dims:
        raise ValueError(
            f"Both vectors must have the same number of dimensions for a dot product. "
            f"Got vector_1: {vector_1.dims}, vector_2: {vector_2.dims}."
        )
...
def cross(vector_1: Vector, vector_2: Vector) -> Vector:
    ...
    if not (vector_1.dims == 3 and vector_2.dims == 3):
        raise ValueError(
            f"Both vectors must be 3-dimensional for a cross product. "
            f"Got vector_1: {vector_1.dims}, vector_2: {vector_2.dims}."
        )
